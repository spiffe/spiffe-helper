...
func (s *sidecarTest) NewSidecar(_ *testing.T) {
	...
	s.sidecar = &Sidecar{
		...
		cmdExitChan:          make(chan os.ProcessState, 1),
		pidFileSignalledChan: make(chan pidFileSignalledResult, 1),
	}
...
// Close the sidecar instance and its channels. Not really needed
// but it's cleaner.
func (s *sidecarTest) Close() {
	close(s.sidecar.certReadyChan)
	close(s.sidecar.cmdExitChan)
	close(s.sidecar.pidFileSignalledChan)
}
