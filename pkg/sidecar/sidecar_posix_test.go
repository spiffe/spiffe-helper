...
func SignalListenerHelperMain(_ *testing.M) int {
	if len(os.Args) < 3 || len(os.Args) > 4 {
		fmt.Fprintf(os.Stderr, "Usage: %s signal <signal> [<forward_to_pid>]\n", os.Args[0])
		return 1
	}
	signame := os.Args[2]
	...
	if len(os.Args) >= 4 {
		...
		fmt.Fprintf(os.Stderr, "Forwarding %s to %d\n", signame, forwardToPid)
	}
	...
	fmt.Fprintf(os.Stderr,
		"signal handler with pid %d for %s (%v) listening\n",
		...
		case gotsig := <-c:
			...
			if forwardToProc != nil {
				if err := forwardToProc.Signal(gotsig); err != nil {
					fmt.Fprintf(os.Stderr, "Failed to forward signal %s to %d: %v\n", gotsig, forwardToPid, err)
				} else {
					fmt.Fprintf(os.Stderr, "forwarded %s to %d\n", gotsig, forwardToPid)
				}
			}
		case <-ctx.Done():
...
func TestSidecar_TestCmdRunsLongRunning(t *testing.T) {
	...
	config.CmdArgs = fmt.Sprintf("signal %s %d", SignalName(testsig), os.Getpid())
	...
	require.Equal(t, int32(1), sidecar.processRunning)
